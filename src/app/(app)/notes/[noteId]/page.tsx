...
export default async function NotePage({ params }: { params: Promise<{ noteId: string }> }) {
  ...
  const inspector = <div className="text-sm">
    ...
    <section>
      <h2 className="text-xs font-medium text-[var(--text-tertiary)]">文件夹</h2>
      <form action={moveNote} className="mt-2 grid gap-2">
        ...
        <button className="justify-self-start text-xs text-[var(--accent)] hover:underline">移动笔记</button>
      </form>
    </section>
    <NoteBacklinks referenced={data.links} backlinks={data.backlinks} />
    <section className="mt-7">
      <h2 className="text-xs font-medium text-[var(--text-tertiary)]">Versions</h2>
      <form action={createNoteVersion} className="mt-2"><input type="hidden" name="note_id" value={data.note.id} /><button className="text-xs text-[var(--accent)] hover:underline">创建版本</button></form>
      <div className="mt-3 space-y-2 text-xs text-[var(--text-secondary)]">{data.versions.map((version) => <form action={restoreNoteVersion} key={version.id} className="flex gap-2"><span>v{version.version_number} · {version.reason}</span><input type="hidden" name="note_id" value={data.note.id} /><input type="hidden" name="version_id" value={version.id} /><button className="text-[var(--accent)]">恢复</button></form>)}</div>
    </section>
    <details className="mt-8 border-t pt-4">
      <summary className="cursor-pointer text-xs font-medium text-[var(--text-tertiary)]">Advanced</summary>
      <a className="mt-4 block text-[var(--accent)]" href={`/api/exports/notes/${data.note.id}`}>下载 Markdown</a>
      <form action={archiveNote} className="mt-4"><input type="hidden" name="note_id" value={data.note.id} /><button className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">归档</button></form>
