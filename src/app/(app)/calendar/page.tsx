...
export default async function CalendarPage() {
  ...
  return <section className="max-w-none"><header className="flex flex-wrap items-start justify-between gap-4 border-b pb-5"><div><p className="text-xs font-medium tracking-wide text-zinc-500">PLANNING</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">Calendar</h1><p className="mt-1 text-sm text-zinc-500">Outlook 是同步端；日程同时保存在你的 Supabase 私有云备份中。</p></div>{connection ? <form action={syncAndBackupMicrosoftAction}><button className="inline-flex items-center gap-1.5 border px-3 py-2 text-sm text-[#365F78]"><RefreshCw size={16} />立即对齐并备份</button></form> : null}</header>{!connection || connection.last_error_code === "calendar_not_connected" ? <MicrosoftDeviceConnect reconnect={Boolean(connection)} /> : <><CalendarWorkspace events={events} /><p className="mt-3 text-xs text-zinc-500">显示当前前后数月的已同步 Outlook 日程。立即对齐会同时备份日程与 To Do；云端每天还会执行一次低频备份。</p></>}</section>;
}
