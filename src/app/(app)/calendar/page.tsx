...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ create?: string }> }) {
  const [{ connection, events, timezone, unavailable }, params] = await Promise.all([getCalendarWorkspace(), searchParams]);
  ...
  return <CalendarWorkspace events={events} timezone={timezone} initialCreateOpen={params.create === "1"} />;
}
