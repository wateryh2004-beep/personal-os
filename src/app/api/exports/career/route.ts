...
export async function GET() {
  try {
    ...
    return new NextResponse(JSON.stringify(body, null, 2), { headers: { "content-type": "application/json; charset=utf-8", "content-disposition": "attachment; filename=career-export.json", "cache-control": "private, no-store, max-age=0" } });
  } catch (error) {
    const authFailure = apiAuthenticationFailure(error);
    if (authFailure) return authFailure;
    return NextResponse.json({ error: "无法生成导出。" }, { status: 500, headers: { "cache-control": "private, no-store, max-age=0" } });
  }
}
