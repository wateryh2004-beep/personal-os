...
export async function GET(request: NextRequest) {
  ...
  if (cronRun) await admin.from("calendar_sync_cron_runs").update({ completed_at: new Date().toISOString(), connection_count: (connections ?? []).length, succeeded_count: Math.max(0, results.length - failed), failed_count: failed, duration_ms: Date.now() - startedAt.getTime(), stage_durations_ms: { queue: queueDurationMs, full_reconcile: reconcileDurationMs }, next_scheduled_at: nextScheduledAt, error_code: failed ? "partial_failure" : null }).eq("id", cronRun.id);
