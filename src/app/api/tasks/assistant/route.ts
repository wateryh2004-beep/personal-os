...
import { createTaskAgent } from "@/lib/ai/task-agent";
...
function safeErrorMessage(error: unknown) {
  ...
  if (statusCode === 401 || value.includes("api key") || value.includes("authentication")) return "DeepSeek API Key 无效或已失效。请在 Settings 重新保存。";
  if (statusCode === 402 || value.includes("insufficient") || value.includes("balance")) return "DeepSeek 账户余额不足，请在 DeepSeek 平台充值后重试。";
  if (statusCode === 429 || value.includes("rate limit")) return "DeepSeek 当前请求过于频繁，请稍后重试。";
  if (value.includes("timeout") || value.includes("abort")) return "DeepSeek 响应超时，请重试。";
  return "DeepSeek 暂时不可用，请稍后重试。";
}
...
export async function POST(request: Request) {
  try {
    ...
    const message = error instanceof Error && error.message === "deepseek_not_configured"
      ? "请先在 Settings 保存 DeepSeek API Key。"
      : "AI 任务助手暂时不可用，请检查 DeepSeek 设置后重试。";
