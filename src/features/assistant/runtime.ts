...
import { assertAiBudget, auditAiRequest, completeAiRequestWithUsage, getAiGovernance, summarizeContextSources } from "@/features/ai/governance";
...
async function setup(request:AssistantRequest) {
  const {supabase,userId}=await requireOwner(); const {data:profile}=await supabase.from("profiles").select("timezone, display_name").eq("user_id",userId).maybeSingle(); const timezone=profile?.timezone||"Asia/Shanghai"; const userName=profile?.display_name?.trim()||"Hang Yu"; const message=latestText(request); const now=new Date(); const policy=resolveAssistantPolicy(request); const governance=await getAiGovernance(userId);
  ...
  if(!budget.allowed) {
    await auditAiRequest({userId,runId:request.runId,surface:request.surface,purpose:request.operation??request.mode,status:"blocked_budget",retrievalMode:gate.mode==="none"?"none":gate.mode==="local"?"local":"targeted",sourceSummary,retrievalReason:personalContextPack?.plan.expansionReason,contextChars:sourceSummary.sourceCount?personalContextPack?.diagnostics.totalChars??0:0,outputTokenLimit:governance.maxOutputTokensPerRequest,errorCode:budget.code});
  ...
  const initial=gate.needsTools
    ...
    : [];
  const auditId=await auditAiRequest({userId,runId:request.runId,surface:request.surface,purpose:request.operation??request.mode,model:resolved.modelId,status:"allowed",retrievalMode:gate.mode==="none"?"none":gate.mode==="local"?"local":personalContextPack?.plan.expansionReason?"expanded":"targeted",sourceSummary,retrievalReason:personalContextPack?.plan.expansionReason,contextChars:personalContextPack?.diagnostics.totalChars??0,outputTokenLimit:governance.maxOutputTokensPerRequest});
  if(request.runId) { await updateAgentRun({supabase,userId,runId:request.runId,status:"running",model:resolved.modelId,kernel:{contextMode:gate.mode,complexity:gate.complexity,initialModules:gate.likelyModules,activeSkills:sessionState.activeSkills,initialToolNames:initial,discoveredToolNames:sessionState.discoveredToolNames,sessionState}}); await recordAgentStep({supabase,userId,runId:request.runId,stepType:"context",title:gate.needsPersonalData?"已准备个人上下文":"未访问 Personal OS",summary:gate.needsPersonalData?`模式 ${gate.mode}；来源 ${sourceSummary.sourceCount} 条（${personalContextPack?.diagnostics.totalChars??0} 字符）`:"普通问题直接回答",output:{contextMode:gate.mode,complexity:gate.complexity,initialModules:gate.likelyModules,activeSkills:sessionState.activeSkills,initialToolNames:initial,personalDataAccessed:Boolean(personalContextPack),osManifestChars:system.length,skillInstructionsChars:0,contextChars:personalContextPack?.diagnostics.totalChars??0,sourceSummary,sources:(personalContextPack?.sources??[]).map(({id,title,domain,href})=>({id,title,domain,href}),),retrievalReason:personalContextPack?.plan.expansionReason,auditId}}); }
  return {supabase,userId,timezone,policy,gate,sessionState,model:resolved.model,modelId:resolved.modelId,system,currentSurface,initial,personalContextPack,auditId,maxOutputTokens:governance.maxOutputTokensPerRequest,governance};
}
