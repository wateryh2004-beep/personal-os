...
import { assertAiBudget, auditAiRequest, completeAiRequest, getAiGovernance, summarizeContextSources } from "@/features/ai/governance";
...
async function setup(request:AssistantRequest) {
  ...
  return {supabase,userId,timezone,policy,gate,sessionState,model:resolved.model,modelId:resolved.modelId,system,currentSurface,initial,personalContextPack,auditId,maxOutputTokens:governance.maxOutputTokensPerRequest};
}
...
export async function runAssistant(request:AssistantRequest):Promise<AssistantResult> { const runtime=await setup(request); try { const {text,finishReason}=await generateText({model:runtime.model,maxOutputTokens:runtime.maxOutputTokens,providerOptions:selectReasoningProviderOptionsForRequest({surface:request.surface,mode:request.mode,operation:request.operation,contextGate:runtime.gate}),system:runtime.system,prompt:latestText(request)}); await completeAiRequest(runtime.auditId,"completed"); await runtime.supabase.from("audit_logs").insert({user_id:runtime.userId,action:"assist",entity_type:request.currentEntity?.type||"assistant",entity_id:request.currentEntity?.id??null,actor_type:"user",after_data:{surface:request.surface,mode:request.mode,model:runtime.modelId,context_mode:runtime.gate.mode,personal_data_accessed:Boolean(runtime.personalContextPack),context_sources:runtime.personalContextPack?.sources.length??0,tool_names:[],finish_reason:finishReason}}); return {status:"success",text:text.trim(),finishReason,modelId:runtime.modelId,contextSources:mapPersonalContextSources(runtime.personalContextPack)}; } catch(error) { await completeAiRequest(runtime.auditId,"failed",error instanceof Error?error.message:"ai_request_failed"); throw error; } }
