  ...
  build: (context) => ({
    searchFiles: tool({
      ...
      execute: async ({ query, limit }) => {
        ...
        const { data, error } = await context.supabase.from("documents").select(`${metadataSelect},extracted_text`).or(`title.ilike.%${escaped}%,original_filename.ilike.%${escaped}%,extracted_text.ilike.%${escaped}%`).eq("storage_state", "available").eq("ai_visibility", "normal").is("archived_at", null).order("updated_at", { ascending: false }).limit(limit);
    ...
    readFileMetadata: tool({
      ...
      execute: async ({ documentId }) => {
        const { data, error } = await context.supabase.from("documents").select(metadataSelect).eq("id", documentId).eq("storage_state", "available").eq("ai_visibility", "normal").is("archived_at", null).maybeSingle();
    ...
    readFileText: tool({
      ...
      execute: async ({ documentId, maxChars }) => {
        const { data, error } = await context.supabase.from("documents").select("id,title,original_filename,text_extraction_status,extracted_text,updated_at").eq("id", documentId).eq("storage_state", "available").eq("ai_visibility", "normal").is("archived_at", null).maybeSingle();
