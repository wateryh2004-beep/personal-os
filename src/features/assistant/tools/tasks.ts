...
import { todoCompleteProposalSchema } from "./schemas";
...
export const taskTools: AssistantToolModule = {
  definitions: [
    ...
    { name: "proposeTodoTask", group: "todo_proposal", risk: "proposal", description: "创建任务提案" },
    ...
    searchTodoTasks: tool({
      ...
      execute: async ({ query, status, dueBefore, limit }) => {
        let request = context.supabase
          ...
          .select("id,title,body_text,status,due_at,importance,completed_at,todo_list_id,microsoft_todo_lists(display_name)")
          .is("archived_at", null)
    ...
    proposeTodoComplete: tool({
      ...
      execute: async (proposal) => {
        const { data } = await context.supabase
          ...
          .eq("status", proposal.expectedStatus)
          .is("archived_at", null)
          .maybeSingle();
