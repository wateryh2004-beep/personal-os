...
export async function generateNoteAiSuggestion(_previous: NoteAiState, formData: FormData): Promise<NoteAiState> {
  ...
  try {
    ...
    await supabase.from("audit_logs").insert({ user_id: userId, action: "assist", entity_type: "note", entity_id: parsed.data.noteId, actor_type: "user", after_data: { provider: "deepseek", operation: "note_assist", instruction_length: parsed.data.instruction.length } });
