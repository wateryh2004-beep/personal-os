...
const instructions: Record<NoteAiOperation, string> = {
  ...
  generateTitle: "根据整篇笔记的内容与语气，为它生成一个标题。好标题要能让人一眼知道这篇写了什么（核心主题或结论），同时保留原文的语气与个人声音：随想、复盘、日记类笔记的标题可以带态度、口语化、有记忆点，不必写成四平八稳的书面标题，也要避免『关于…的思考』『…的复盘』『论…』这类模板化标题。长度以准确传神为准，没有硬性字数限制，通常 6~20 字即可，需要时可以更短或更长。示例：一篇写『因为缺几千块钱而烦躁，并反思各种赚钱途径』的随想，好标题是『几千块钱就让我烦躁，是我无能吗』，而不是『关于赚钱问题的思考』。只输出标题本身，不加引号、冒号、序号、解释或任何 Markdown 格式。",
