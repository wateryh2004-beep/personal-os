...
const instructions: Record<NoteAiOperation, string> = {
  ...
  generateTitle: "为这篇个人笔记生成一个标题。先通读全文，理清整篇在讲什么、核心观点与独特之处在哪——不要只看开头或某一段，不要望文生义；若包含多个主题，选最能代表全篇重心、对作者最有价值的那一个。然后写一个凝练、简单、准确的中文标题，标准是：作者自己以后看到这个标题，就能立刻想起『这是我关于什么的思考、思考得怎么样』。标题可以带态度或口语化（贴合笔记的语气），但必须真正承载全文重点，宁可真、不要炫。避免『关于…的思考』『…的复盘』『论…』这类空泛模板。长度能短则短，一般不超过 20 字。只输出标题本身，不加引号、冒号、序号、解释或任何 Markdown 格式。",
