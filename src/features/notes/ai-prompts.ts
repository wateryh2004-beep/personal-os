...
const instructions: Record<NoteAiOperation, string> = {
  summarizeNote: "仅依据当前笔记。先用一两句话说清这篇笔记最核心的一层意思，再列最多 5 条关键信息。保留重要人名、日期、数字、具体判断和作者真实的矛盾点；不要用「本文探讨了…」「总而言之」这类总结腔，宁可用笔记里的原话和具体细节。存在未解决问题就单列“待确认”。短笔记不要强行扩写。",
  extractActions: "只提取原文明确存在或可非常直接推导出的行动，不制造任务。每条行动要具体可执行——谁、做什么、做到什么程度；不要「跟进」「推进」「保持沟通」这类万能动词，写成能直接安排进日程的句子。有截止时间才写截止时间；没有行动时明确说明。使用 Markdown checklist。",
  restructureNote: "保留全部事实与原意，重新组织标题、段落和层级，使结构更清楚。各级标题用笔记里本来就有的具体词，不要凭空造「引言」「总结」这类空标题；不新增观点或结论。仅输出可替换的 Markdown 正文。",
  polishNote: "目标是让文字更顺、更好读、更清晰。可以调整语序、重组段落、重排事件顺序，但原文里说到的每一件事、观点和细节都必须保留在结果里：不得删除任何完整段落、主题或信息点，只允许删掉句内明显重复的词。不增加新事实，不改变专有名词、日期或数字。保留第一人称语气与口语原声（停顿、语气词、吐槽、未说完的话），不要把真实、粗粝或尚未想清楚的内容强行改成成熟结论。润色不是重写：能改的词少改，作者自己的说话节奏尽量不动。仅输出可替换的 Markdown 正文。",
  deepThinkNote: "仅基于当前笔记，识别关键判断、隐含假设、因果跳跃、可能反例、风险与待确认问题。必须区分原文事实和推论；不要迎合原结论——哪里站不住、哪里用情绪代替了论证，直接指出来；也不要补充原文没有依据的事实。优先追问具体证据和替代解释，最后把仍应由 Hang Yu 判断的问题单列出来。",
  generateTitle: "为这篇个人笔记生成一个最好的标题。先在心中写下 5 个角度各异的候选，务必用 5 种互不相同的句式，逐一按标准检验后，选出最贴切的那个作为最终标题输出。\n句式多样性是硬性要求：候选里不许出现两个同结构。允许的形态有——一句原话（笔记里最有冲击力的一句判断）、一个具体场景或意象、一个设问（以驱动这篇笔记的问题开头）、一个有态度的断言、一个干净的主题短语、一句口语短话。对仗式（如「选择，还是行动」）也算一种形态，但 5 个里最多 1 个，且不要让它当首选。\n检验标准：\n· 准确——标题必须钉在这篇笔记的核心上，换个标题安到别的笔记上就不成立；\n· 记忆点——Hang Yu 以后看到它，能立刻想起「这篇在讲什么、为什么值得我写」；\n· 个人气味——像他自己会起的，不像 AI 腔；宁可真、不要炫，一般不超过 20 字。\n反例（太空泛、没有个人味，禁止）：「关于时间的思考」「努力与选择」「向内生长，向外绽放」——这些安到任何一篇笔记上都成立。\n只输出最终选中的标题本身。不要序号、引号、候选列表或任何解释。",
  extractKeyInsights: "仅依据当前笔记，提炼 3–5 条真正值得记住的洞见或原句金句。只有反常识、反复出现、或带强烈个人判断的内容才配提炼——普通事实和信息流水账不要硬拔成「金句」。有现成原句能概括的就原样引用，否则用一两句平实的话概括。避免「生活就是…」「一切都是…」式空泛格言。只输出 Markdown 列表，不要开场白。",
