...
export async function getCalendarWorkspace(owner?: Owner) {
  ...
  const sync = connection.data ? (() => {
    ...
    const state = connection.data.last_error_code ? "failed" : ageMs <= 3600_000 ? "fresh" : ageMs <= 2 * 3600_000 ? "stale" : "stale";
    return { state, lastSyncAt, nextHourlyAt, nextFullAt, subscriptionExpiresAt: connection.data.calendar_subscription_expires_at, webhookLastReceivedAt: connection.data.calendar_webhook_last_received_at, errorCode: connection.data.last_error_code };
  })() : null;
