...
export async function getCalendarWorkspace() {
  ...
  const [connection, events, profile] = await Promise.all([
  ...
  return { connection: connection.data, events: events.data ?? [], timezone: profile.data?.timezone || "Asia/Shanghai", unavailable: Boolean(connection.error || events.error) };
}
