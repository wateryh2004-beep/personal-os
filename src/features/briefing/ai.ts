...
export type BriefingAiResult = { selected: Array<RankedCandidate & { ai?: AiEvaluation; summary?: string; relevanceReason?: string }>; method: "ai_hybrid" | "deterministic_fallback"; model: string | null; calls: number; inputTokens: number; outputTokens: number; usageReported: boolean };
...
export async function evaluateBriefingWithAi({ supabase, userId, candidates, interests, now = new Date() }: { supabase: SupabaseClient; userId: string; candidates: FeedCandidate[]; interests: BriefingInterest[]; now?: Date }): Promise<BriefingAiResult> {
  const { data: settings } = await supabase.from("briefing_settings").select("ai_enabled,max_ai_candidates,max_selected_items,daily_input_token_budget,budget_exhaustion_behavior").eq("user_id", userId).maybeSingle();
  ...
  const fallback = () => ({ selected: diversifyCandidates(prefilterBriefingCandidates(candidates, interests, now, maxCandidates), maxSelected), method: "deterministic_fallback" as const, model: null, calls: 0, inputTokens: 0, outputTokens: 0, usageReported: false });
  if (settings?.ai_enabled === false) return fallback();
  const prefiltered = prefilterBriefingCandidates(candidates, interests, now, maxCandidates); if (!prefiltered.length) return fallback();
  ...
  const used = (usageRows ?? []).reduce((total, row) => total + Number(row.input_tokens ?? 0), 0); if (used >= (settings?.daily_input_token_budget ?? 20000)) { if (settings?.budget_exhaustion_behavior === "pause") throw new Error("ai_budget_exhausted"); return fallback(); }
  try {
    const { model, modelId } = await getDeepSeekModel(userId, "deepseek-v4-flash");
    ...
    const missing = prefiltered.filter((item) => !cachedById.has(item.itemId)); let calls = 0, inputTokens = 0, outputTokens = 0, usageReported = false;
    ...
    const evaluated = prefiltered.map((item) => ({ ...item, ai: cachedById.get(item.itemId) })).filter((item): item is RankedCandidate & { ai: AiEvaluation } => Boolean(item.ai)); const selected = selectDiverseAiCandidates(evaluated, maxSelected); if (!selected.length) return fallback();
    ...
    return { selected: selected.map((item) => ({ ...item, summary: summaries.get(item.itemId)?.summary ?? item.excerpt.slice(0, 400), relevanceReason: summaries.get(item.itemId)?.relevanceReason ?? item.ai?.reason ?? item.reason })), method: "ai_hybrid", model: modelId, calls, inputTokens, outputTokens, usageReported };
