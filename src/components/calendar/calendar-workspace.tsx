...
const subscribeToMobileCalendar = (notify: () => void) => {
  const query = window.matchMedia(mobileCalendarQuery);
  query.addEventListener("change", notify);
  return () => query.removeEventListener("change", notify);
};
const getMobileCalendarSnapshot = () => window.matchMedia(mobileCalendarQuery).matches;
const getMobileCalendarServerSnapshot = () => false;
...
function TimeGrid({ dates, segmentsByDate, today, timezone, categories, view, now, onOpen, onCreate }: { dates: Date[]; segmentsByDate: Map<string, TimedEventSegment<CalendarEventRecord>[]>; today: string; timezone: string; categories: CalendarCategory[]; view: "day" | "week"; now: Date; onOpen: (event: CalendarEventRecord) => void; onCreate: (range: DraftRange) => void }) {
  ...
  const dateKeys = dates.map(keyOf).join(",");
  const columns = `${timeColumnWidth}px repeat(${dates.length}, minmax(${singleDay ? "0" : "145px"}, 1fr))`;
  ...
  useEffect(() => {
    ...
    if (!singleDay && window.matchMedia("(max-width: 767px)").matches) {
      const todayIndex = dateKeys.split(",").indexOf(today);
      if (todayIndex >= 0) {
        const dayWidth = (scrollRef.current.scrollWidth - timeColumnWidth) / dates.length;
        scrollRef.current.scrollLeft = Math.max(0, timeColumnWidth + todayIndex * dayWidth - 8);
      }
    }
  }, [dateKeys, dates.length, singleDay, today, timezone]); // Initial viewport only; the live line moves without stealing scroll.
  return <div ref={scrollRef} className="workspace-scroll min-h-0 flex-1 overflow-auto">
    <div style={{ minWidth: singleDay ? "100%" : 820 }}>
...
export function CalendarWorkspace({ events, categories, timezone, scopeReady, initialCreateOpen = false }: { events: CalendarEventRecord[]; categories: CalendarCategory[]; timezone: string; scopeReady: boolean; initialCreateOpen?: boolean }) {
  ...
  const isMobile = useSyncExternalStore(subscribeToMobileCalendar, getMobileCalendarSnapshot, getMobileCalendarServerSnapshot);
  const [preferredView, setView] = useState<CalendarView | null>(null);
  const view = preferredView ?? (isMobile ? "day" : "week");
  ...
  return <section className="flex h-[calc(var(--app-viewport-height)-var(--toolbar-height))] min-h-0 overflow-hidden bg-white md:min-h-[540px]">
    <div className="flex min-w-0 flex-1 flex-col">
      <div className="relative shrink-0 border-b px-2 py-2 sm:flex sm:min-h-[var(--toolbar-height)] sm:items-center sm:justify-between sm:gap-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-1">
          <button className="rounded-md p-2 hover:bg-[var(--surface-hover)]" onClick={() => step(-1)} aria-label="上一段日期"><ChevronLeft size={16}/></button>
          <button className="rounded-md px-2.5 py-1.5 text-sm hover:bg-[var(--surface-hover)]" onClick={() => setCursor(startOfDay(new Date()))}>今天</button>
          <button className="rounded-md p-2 hover:bg-[var(--surface-hover)]" onClick={() => step(1)} aria-label="下一段日期"><ChevronRight size={16}/></button>
          <h1 className="ml-1 min-w-0 flex-1 truncate text-sm font-semibold sm:ml-2">{title}</h1>
        </div>
        <div className="workspace-scroll mt-1 flex items-center gap-1 overflow-x-auto pb-0.5 sm:mt-0 sm:overflow-visible sm:pb-0">
          <div className="flex shrink-0 rounded-md bg-[var(--surface-hover)] p-0.5">{(["day", "week", "month"] as const).map((item) => <button key={item} onClick={() => setView(item)} aria-label={`${item === "day" ? "日" : item === "week" ? "周" : "月"}视图`} aria-pressed={view === item} className={`min-h-8 rounded px-3 py-1 text-xs ${view === item ? "bg-white text-[var(--accent)]" : "text-[var(--text-secondary)]"}`}>{item === "day" ? "日" : item === "week" ? "周" : "月"}</button>)}</div>
          <button type="button" disabled={isSyncing} onClick={sync} aria-label={isSyncing ? "正在对齐 Outlook" : "对齐 Outlook"} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-md px-2.5 py-1.5 text-xs text-[var(--accent)] hover:bg-[var(--surface-hover)] disabled:opacity-60"><RefreshCw size={14} className={isSyncing ? "animate-spin" : ""}/><span className="hidden sm:inline">{isSyncing ? "对齐中…" : "对齐"}</span></button>
          <button onClick={() => setFilterOpen((value) => !value)} aria-label="筛选日历分类" aria-pressed={filterOpen || selectedCategories.size > 0} className={`inline-flex min-h-8 shrink-0 items-center gap-1 rounded-md px-2.5 py-1.5 text-xs hover:bg-[var(--surface-hover)] ${selectedCategories.size ? "text-[var(--accent)]" : "text-zinc-600"}`}><Filter size={14}/><span className="hidden sm:inline">{selectedCategories.size || "分类"}</span>{selectedCategories.size ? <span className="sm:hidden">{selectedCategories.size}</span> : null}</button>
          <button onClick={() => setSettingsOpen(true)} className="min-h-8 shrink-0 rounded-md p-2 text-zinc-500 hover:bg-[var(--surface-hover)]" aria-label="日历设置"><Settings2 size={15}/></button>
          <button onClick={calendarAssistant.toggle} aria-pressed={calendarAssistant.isOpen} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-md px-2.5 py-1.5 text-xs text-[var(--accent)] hover:bg-[var(--surface-hover)]"><Bot size={14}/><span className="hidden sm:inline">AI</span></button>
          <button onClick={() => openCreate()} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-md bg-[var(--accent)] px-3 py-1.5 text-xs text-white"><Plus size={14}/>新建</button>
        </div>
        {filterOpen ? <div className="absolute inset-x-2 top-[calc(100%+4px)] z-50 rounded-md border bg-white p-2 shadow-sm sm:left-auto sm:right-24 sm:w-64"><div className="mb-1 flex items-center justify-between px-2 py-1"><p className="text-xs font-medium">按 Outlook 分类筛选</p>{selectedCategories.size ? <button onClick={() => setSelectedCategories(new Set())} className="text-[10px] text-[var(--accent)]">清除</button> : null}</div><div className="max-h-72 overflow-auto">{categories.map((category) => { const visual = resolveCalendarEventVisual([category.display_name], categories); const selected = selectedCategories.has(category.display_name); return <button key={category.id} onClick={() => toggleCategory(category.display_name)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-xs hover:bg-zinc-50"><span className="size-2 rounded-full" style={{ background: visual.dot }}/><span className="min-w-0 flex-1 truncate">{category.display_name}</span>{selected ? <Check size={13} className="text-[var(--accent)]"/> : null}</button>; })}{!categories.length ? <p className="px-2 py-4 text-center text-xs text-zinc-500">尚未同步分类</p> : null}</div></div> : null}
      </div>
      {!scopeReady ? <button type="button" onClick={() => setSettingsOpen(true)} className="border-b border-amber-200 bg-amber-50 px-3 py-2 text-left text-xs text-amber-900 sm:px-4">Outlook 分类需要重新授权。日程仍可使用，现有分类不会被覆盖。点击处理。</button> : null}
      {syncState.message ? <p role="status" aria-live="polite" className={`min-h-6 border-b px-3 py-1 text-xs sm:px-4 ${syncState.tone === "error" ? "bg-red-50 text-red-700" : syncState.tone === "success" ? "bg-[var(--accent-soft)] text-[var(--accent)]" : "text-[var(--text-tertiary)]"}`}>{syncState.message}</p> : null}
      {view === "month" ? <div className="workspace-scroll min-h-0 flex-1 overflow-auto"><div className="grid h-full min-w-0 grid-cols-7 grid-rows-[auto_repeat(6,minmax(62px,1fr))] sm:min-w-[680px] sm:grid-rows-[auto_repeat(6,minmax(92px,1fr))]">{weekDays.map((day) => <div key={day} className="sticky top-0 z-20 border-b bg-white px-1 py-2 text-center text-[11px] text-[var(--text-secondary)] sm:px-2 sm:text-xs">周{day}</div>)}{dates.map((date) => { const key = keyOf(date); const daySegments = segmentsByDate.get(key) ?? []; const dayEvents = Array.from(new Map(daySegments.map((segment) => [segment.event.id, segment.event])).values()); const weekend = date.getDay() === 0 || date.getDay() === 6; const openDay = () => { setCursor(date); setView("day"); }; return <div role="button" tabIndex={0} key={key} onClick={openDay} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openDay(); }} className={`min-h-0 border-b border-r p-1 text-left align-top outline-none hover:bg-zinc-50/70 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--accent)] sm:p-1.5 ${date.getMonth() !== cursor.getMonth() ? "bg-[var(--surface-app)] text-[var(--text-tertiary)]" : weekend ? "bg-zinc-50/40" : ""}`}><span className={`mb-1 flex size-6 items-center justify-center text-xs ${key === today ? "rounded-full bg-[var(--accent)] text-white" : ""}`}>{date.getDate()}</span><span className="flex items-center gap-0.5 px-0.5 sm:hidden">{dayEvents.slice(0, 3).map((event) => <span key={event.id} className="size-1.5 rounded-full" style={{ background: resolveCalendarEventVisual(event.categories, categories).dot }} />)}{dayEvents.length > 3 ? <span className="ml-0.5 text-[9px] text-[var(--text-tertiary)]">+{dayEvents.length - 3}</span> : null}</span><span className="hidden space-y-0.5 overflow-hidden sm:block">{dayEvents.slice(0, 4).map((event) => <MonthEventButton key={event.id} event={event} timezone={timezone} categories={categories} onOpen={() => openEvent(event)} />)}{dayEvents.length > 4 ? <span className="block px-1 text-[10px] text-[var(--text-secondary)]">+{dayEvents.length - 4} 项</span> : null}</span></div>; })}</div></div> : <TimeGrid dates={dates} segmentsByDate={segmentsByDate} today={today} timezone={timezone} categories={categories} view={view} now={now} onOpen={openEvent} onCreate={openCreate} />}
    </div>
    <CalendarInspector open={calendarInspectorOpen} event={selectedEvent} draft={draft} timezone={timezone} categories={categories} categoriesEnabled={scopeReady} onClose={closeInspector}/>
    <AISidecar open={calendarAssistant.isOpen} onClose={calendarAssistant.close} context="Calendar"><CalendarAssistant timezone={timezone} categories={categories}/></AISidecar>
    <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent className="max-h-[calc(var(--app-viewport-height)-2rem)] overflow-y-auto bg-white p-4 sm:max-w-xl sm:p-6"><div className="border-b pb-4"><h2 className="text-lg font-semibold">日历分类</h2><p className="mt-1 text-sm text-zinc-500">与 Outlook Master Categories 双向保持一致。</p></div><CalendarCategoryManager categories={categories} scopeReady={scopeReady} events={normalizedEvents} referenceTime={now.getTime()}/></DialogContent></Dialog>
  </section>;
}
