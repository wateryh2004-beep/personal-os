...
function TimeGrid({ dates, eventsByDate, today }: { dates: Date[]; eventsByDate: Map<string, CalendarEvent[]>; today: string }) {
  ...
  return <div className="min-w-[820px]" style={{ gridTemplateColumns: columns }}>
    ...
    <div className="grid" style={{ gridTemplateColumns: columns }}>
      ...
      {dates.map((date) => { const key = keyOf(date); const layouts = layoutTimedEvents(eventsByDate.get(key) ?? [], visibleStartHour, visibleEndHour); return <div key={key} className={`relative border-r last:border-r-0 ${key === today ? "bg-[#F7FAFB]" : ""}`} style={{ height: gridHeight, backgroundImage: "linear-gradient(to bottom, #e7e5e4 1px, transparent 1px)", backgroundSize: `100% ${hourHeight}px` }}>{layouts.map(({ event, startMinutes, endMinutes, column, columns: columnCount }) => <TimedEvent key={event.id} event={event} top={((startMinutes - visibleStartHour * 60) / 60) * hourHeight} height={Math.max(22, ((endMinutes - startMinutes) / 60) * hourHeight)} column={column} columns={columnCount} />)}</div>; })}
    </div>
  </div>;
}
