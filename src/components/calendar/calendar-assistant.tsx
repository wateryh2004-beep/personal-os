...
export function CalendarAssistant() {
  ...
  return <section className="mt-8 border-t pt-6"><div><p className="text-xs font-medium tracking-wide text-zinc-500">AI CALENDAR</p><h2 className="mt-1 font-medium">日历助手</h2><p className="mt-1 text-sm text-zinc-500">例如：“明天下午两点到三点和张三开会”或“我这周哪天有空？”</p></div><div className="mt-4 max-h-80 space-y-3 overflow-y-auto pr-1">{!messages.length ? <p className="text-sm text-zinc-500">AI 只生成提案；你点击一次“确认创建日程”后才会写入 Outlook。</p> : messages.map((message) => <div key={message.id} className={message.role === "user" ? "ml-8 text-right" : "mr-8"}>{message.parts.map((part, index) => { if (part.type === "text") return <p key={index} className={message.role === "user" ? "inline-block bg-[#EDF3F6] px-3 py-2 text-sm" : "whitespace-pre-wrap text-sm leading-6 text-zinc-700"}>{part.text}</p>; if (part.type === "tool-proposeCalendarEvent" && part.state === "output-available") { const output = part.output as CalendarProposal; return <CalendarProposalForm key={part.toolCallId} event={output.proposal} />; } return null; })}</div>)}</div>{stoppedMessage ? <p role="status" className="mt-3 text-sm text-zinc-600">{stoppedMessage}</p> : null}{error && !stoppedMessage ? <p role="status" className="mt-3 text-sm text-red-700">AI 暂时没有完成回答。请重试；若持续发生，请检查 DeepSeek Key 与账户余额。</p> : null}<form className="mt-4 flex gap-2" onSubmit={(event) => { event.preventDefault(); if (!input.trim() || waiting) return; clearError(); setStoppedMessage(null); sendMessage({ text: input }); setInput(""); timeoutRef.current = setTimeout(() => stopGeneration("回答超时，已停止。请重试。"), 18_500); }}><label className="sr-only" htmlFor="calendar-ai-input">向日历助手提问</label><input id="calendar-ai-input" value={input} onChange={(event) => setInput(event.target.value)} disabled={waiting} maxLength={2000} className="min-w-0 flex-1 border bg-white px-3 py-2 text-sm" placeholder="问问你的日程，或用自然语言创建日程…" />{waiting ? <button type="button" className="border px-3 py-2 text-sm" onClick={() => stopGeneration("已停止生成。你可以立即发送新的问题。")}>停止</button> : <button className="bg-[#365F78] px-3 py-2 text-sm text-white">发送</button>}</form></section>;
}
