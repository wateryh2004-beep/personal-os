...
export function CalendarEventEditForm({ event, timezone, calendarCategories, categoriesEnabled = true, onReconcile }: { event: EditableEvent; timezone: string; calendarCategories: CalendarCategory[]; categoriesEnabled?: boolean; onReconcile?: (kind: "update" | "delete") => Promise<void> | void }) {
  ...
  if (!editing || state.status === "success") return <div className="grid gap-5"><div><h3 className="text-base font-semibold">{event.subject}</h3><p className="mt-2 text-sm text-zinc-600">{event.is_all_day ? `${startDate}${startDate === inclusiveEndDate ? "" : ` – ${inclusiveEndDate}`}` : `${dateTimeInputValue(event.starts_at, timezone).replace("T", " ")} – ${dateTimeInputValue(event.ends_at, timezone).slice(11)}`}</p>{event.location_name ? <p className="mt-1 text-sm text-zinc-500">{event.location_name}</p> : null}</div>{event.body_text ? <EntityMarkdown body={event.body_text} className="text-sm leading-6" /> : null}<div className="rounded-md bg-zinc-50 px-3 py-2"><p className="flex items-center gap-2 text-xs text-zinc-700"><span className="size-2 rounded-full" style={{ background: visual.dot }}/>{visual.primaryCategory?.display_name ?? "未分类"}</p><p className="mt-1 text-[10px] text-zinc-400">Outlook 已同步 · {new Intl.DateTimeFormat("zh-CN", { timeZone: timezone, month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(event.last_synced_at))}</p></div>{state.status === "success" ? <p role="status" className="text-sm text-[var(--accent)]">{state.message}</p> : <button onClick={() => { setDescription(event.body_text ?? ""); setEditing(true); }} className="justify-self-start rounded-md border px-3 py-1.5 text-sm hover:bg-zinc-50">编辑</button>}<form action={deleteAction} onSubmit={(submitEvent) => { if (!window.confirm("确认从 Outlook 删除这条日程？")) submitEvent.preventDefault(); }} className="border-t pt-4"><input type="hidden" name="provider_event_id" value={event.provider_event_id}/><input type="hidden" name="subject" value={event.subject}/><input type="hidden" name="starts_at" value={event.starts_at}/><input type="hidden" name="ends_at" value={event.ends_at}/><input type="hidden" name="is_all_day" value={event.is_all_day ? "on" : ""}/><button disabled={deleting} className="text-sm text-red-700 disabled:opacity-60">{deleting ? "正在删除…" : "删除这条日程"}</button>{deleteState.status === "error" ? <p role="status" className="mt-2 text-sm text-red-700">{deleteState.message}</p> : null}</form></div>;
  return <div className="grid gap-5"><form action={action} onSubmit={(submitEvent) => { const form = submitEvent.currentTarget; const start = form.elements.namedItem("starts_at"); const end = form.elements.namedItem("ends_at"); if (!(start instanceof HTMLInputElement) || !(end instanceof HTMLInputElement) || !startRef.current?.value || !endRef.current?.value) return; start.value = wallTimeToIso(allDay ? `${startRef.current.value}T00:00` : startRef.current.value, timezone); end.value = wallTimeToIso(allDay ? `${endRef.current.value}T00:00` : endRef.current.value, timezone); }} className="grid gap-4"><input type="hidden" name="provider_event_id" value={event.provider_event_id}/><input type="hidden" name="original_subject" value={event.subject}/><input type="hidden" name="original_starts_at" value={event.starts_at}/><input type="hidden" name="original_ends_at" value={event.ends_at}/><input type="hidden" name="starts_at"/><input type="hidden" name="ends_at"/><input type="hidden" name="preserve_categories" value={categoriesEnabled ? "false" : "true"}/><input type="hidden" name="is_all_day_present" value="true"/>
    <label className="grid gap-1 text-xs text-zinc-600">标题<input name="subject" autoFocus required maxLength={500} defaultValue={event.subject} className="h-10 rounded-md border px-3 text-sm"/></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="is_all_day" checked={allDay} onChange={(input) => setAllDay(input.target.checked)}/> 全天</label><div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-xs text-zinc-600">{allDay ? "开始日期" : "开始"}<input key={`start-${allDay}`} ref={startRef} type={allDay ? "date" : "datetime-local"} required onChange={keepEndAfterStart} defaultValue={allDay ? startDate : dateTimeInputValue(event.starts_at, timezone)} className="h-10 min-w-0 rounded-md border px-3 text-sm"/></label><label className="grid gap-1 text-xs text-zinc-600">{allDay ? "结束日期（不含）" : "结束"}<input key={`end-${allDay}`} ref={endRef} type={allDay ? "date" : "datetime-local"} required defaultValue={allDay ? allDayEndDate : dateTimeInputValue(event.ends_at, timezone)} className="h-10 min-w-0 rounded-md border px-3 text-sm"/></label></div><label className="grid gap-1 text-xs text-zinc-600">地点<input name="location_name" maxLength={500} defaultValue={event.location_name ?? ""} className="h-10 rounded-md border px-3 text-sm"/></label><label className="grid gap-1 text-xs text-zinc-600">说明<MentionTextarea value={description} onChange={setDescription} name="description" maxLength={10000} rows={4} placeholder="输入 @ 引用笔记/任务/日程/文件" className="min-h-20 rounded-md border px-3 py-2 text-sm"/></label><CalendarCategoryPicker defaultCategories={event.categories} enabled={categoriesEnabled} /><details className="rounded-md border p-3"><summary className="cursor-pointer text-sm text-zinc-600">更多选项</summary><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-xs text-zinc-600">显示为<select name="show_as" defaultValue={event.show_as === "unknown" ? "busy" : event.show_as} className="h-9 rounded-md border bg-white px-2 text-sm"><option value="busy">忙碌</option><option value="free">空闲</option><option value="tentative">暂定</option><option value="workingElsewhere">在其他地点工作</option><option value="oof">外出</option></select></label><label className="grid gap-1 text-xs text-zinc-600">重要性<select name="importance" defaultValue={event.importance} className="h-9 rounded-md border bg-white px-2 text-sm"><option value="low">低</option><option value="normal">普通</option><option value="high">高</option></select></label></div></details><div className="flex gap-2"><button type="button" onClick={() => setEditing(false)} className="rounded-md border px-3 py-2 text-sm">取消</button><button disabled={pending} className="rounded-md bg-[var(--accent)] px-4 py-2 text-sm text-white disabled:opacity-60">{pending ? "正在保存…" : "保存更改"}</button></div>{state.status === "error" ? <p role="status" className="text-sm text-red-700">{state.message}</p> : null}</form></div>;
}
