...
export function CalendarFullView({ events, categories, timezone, initialView, initialDate, onOpen, onCreate, onMove, onRangeChange, loadingRange }: {
  ...
  const eventContent = useCallback((info: { event: { title: string; extendedProps: { visual: { dot: string } } }; timeText: string; view: { type: CalendarView } }) => {
    ...
    return <div className="h-full overflow-hidden px-1.5 py-0.5 text-xs leading-4"><p className="line-clamp-2 font-medium">{info.event.title}</p><p className="text-[10px] opacity-70">{info.timeText}</p></div>;
  }, []);
