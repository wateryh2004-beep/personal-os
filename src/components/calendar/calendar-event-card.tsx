...
export function CalendarEventCard({ layout, timezone, categories, hourHeight, visibleStartHour, onOpen }: { layout: TimedEventLayout<CalendarEventRecord>; timezone: string; categories: CalendarCategory[]; hourHeight: number; visibleStartHour: number; onOpen: () => void }) {
  ...
  const showLocation = height >= 56 && layout.width >= 48 && layout.layoutMode !== "cascade";
  const gutter = layout.layoutMode === "cascade" ? 4 : 5;
  const style: CSSProperties = {
    ...
    zIndex: layout.layoutMode === "cascade" ? 10 + layout.lane : 10,
  };
  const open = (click: MouseEvent<HTMLButtonElement>) => { click.stopPropagation(); onOpen(); };
  return <button type="button" onClick={open} onDoubleClick={(event) => event.stopPropagation()} className="group/event absolute overflow-hidden rounded-md border border-l-[3px] border-y-black/5 border-r-black/5 px-2 py-1 text-left text-xs outline-none transition-[filter] hover:z-50 hover:brightness-[0.98] focus-visible:z-50 focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-1" style={style} aria-label={`${event.subject || "无标题日程"}，${timeLabel(event.starts_at, timezone)} 到 ${timeLabel(event.ends_at, timezone)}`}>
    ...
    {showTime ? <span className="mt-0.5 flex items-center gap-1 truncate font-mono text-[10px] opacity-80">{layout.continuesBefore || layout.clippedBefore ? <ArrowUp className="size-2.5 shrink-0" aria-hidden="true" /> : null}{timeLabel(event.starts_at, timezone)} — {timeLabel(event.ends_at, timezone)}{layout.continuesAfter || layout.clippedAfter ? <ArrowDown className="size-2.5 shrink-0" aria-hidden="true" /> : null}</span> : null}
