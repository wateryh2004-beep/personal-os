...
function Navigation({ pathname, collapsed, onNavigate }: { pathname: string; collapsed: boolean; onNavigate?: () => void }) {
  return <nav aria-label="主导航" className="space-y-5">{groups.map((group, groupIndex) => <div key={group.label ?? groupIndex}>
    ...
    <div className="space-y-0.5">{group.items.map(({ name, href, icon: Icon }) => {
      ...
      const link = <Link href={href} onClick={onNavigate} aria-current={active ? "page" : undefined} aria-label={collapsed ? name : undefined} className={cn("relative flex h-9 min-w-0 items-center gap-2.5 rounded-[var(--radius-md)] px-2 text-sm transition-[background-color,color] ui-transition", active ? "bg-[var(--surface-selected)] font-medium text-[var(--accent)] before:absolute before:inset-y-2 before:-left-2 before:w-0.5 before:rounded-full before:bg-[var(--accent)]" : "text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-primary)]", collapsed && "justify-center gap-0 px-0")}><Icon className="size-[17px] shrink-0" aria-hidden="true" /><span className={cn("min-w-0 truncate transition-[width,opacity] ui-transition", collapsed && "w-0 opacity-0")}>{name}</span></Link>;
...
function AppShellInner({ children }: { children: React.ReactNode }) {
  ...
  const desktopSidebar = useMemo(() => <aside style={{ width: desktopWidth }} className="fixed inset-y-0 left-0 z-30 hidden shrink-0 flex-col border-r bg-[var(--surface-sidebar)] transition-[width] ui-panel-transition md:flex">
  ...
  return <div className="min-h-[var(--app-viewport-height)] bg-[var(--surface-app)]">{desktopSidebar}
    ...
    <div style={{ "--shell-width": desktopWidth } as React.CSSProperties} className="min-w-0 transition-[margin-left] ui-panel-transition md:ml-[var(--shell-width)]">
      <header className="sticky top-0 z-20 grid h-[var(--toolbar-height)] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b bg-[color:var(--surface-canvas)]/95 px-3 backdrop-blur-sm sm:px-4 md:grid-cols-[minmax(0,1fr)_minmax(0,40rem)_minmax(0,1fr)]">
        <div className="flex min-w-0 items-center"><Button variant="ghost" size="icon-sm" className="md:hidden" onClick={() => setMobileOpen(true)} aria-label="打开导航"><Menu aria-hidden="true" /></Button></div>
        <button type="button" onClick={() => openCommand("search")} className="flex h-8 w-full items-center gap-2 rounded-[var(--radius-md)] bg-[var(--surface-hover)] px-3 text-left text-sm text-[var(--text-tertiary)] transition-[background-color,color] ui-transition hover:text-[var(--text-secondary)]"><Search className="size-4 shrink-0" aria-hidden="true" /><span className="min-w-0 flex-1 truncate sm:hidden">搜索…</span><span className="hidden min-w-0 flex-1 truncate sm:inline">搜索 Personal OS…</span><kbd className="hidden rounded-[var(--radius-sm)] border bg-[var(--surface-canvas)] px-1.5 py-0.5 font-sans text-[10px] sm:inline">⌘ K</kbd></button>
        <div className="flex items-center justify-end gap-1"><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="sm" onClick={openGlobalAgent} aria-label="询问 Personal OS" className="gap-1.5"><Sparkles className="size-4" aria-hidden="true"/><span className="hidden sm:inline">Ask</span><kbd className="hidden rounded-[var(--radius-sm)] border bg-[var(--surface-canvas)] px-1 py-0.5 font-sans text-[9px] lg:inline">⌘ J</kbd></Button></TooltipTrigger><TooltipContent>Ask Personal OS</TooltipContent></Tooltip>
        <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon-sm" onClick={() => openCommand("quick")} aria-label="快速新建"><Plus aria-hidden="true" /></Button></TooltipTrigger><TooltipContent>快速新建</TooltipContent></Tooltip></div>
      </header>
