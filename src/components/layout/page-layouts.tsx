...
export function WorkspaceLayout({ className, ...props }: LayoutProps) {
  return <div className={cn("h-[calc(100dvh-var(--toolbar-height))] min-h-[560px] w-full overflow-hidden p-3 sm:p-4", className)} {...props} />;
}
...
export function ContextSidebar({ className, ...props }: React.ComponentProps<"aside">) {
  return <aside className={cn("border-b bg-[var(--surface-sidebar)] p-4 md:sticky md:top-[var(--toolbar-height)] md:h-[calc(100dvh-var(--toolbar-height))] md:overflow-y-auto md:border-r md:border-b-0", className)} {...props} />;
}
