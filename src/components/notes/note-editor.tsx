...
export function NoteEditor({ note, noteAiDefaultModel }: { note: Note; noteAiDefaultModel: DeepSeekModelId }) {
  ...
  return <section ref={editorSurfaceRef} className="notes-editor-surface flex h-full min-w-0 overflow-hidden bg-[var(--surface-canvas)]"><div className="flex min-w-0 flex-1 flex-col"><div className="flex min-h-[var(--toolbar-height)] shrink-0 items-center gap-2 border-b px-3 sm:gap-3 sm:px-6"><input aria-label="笔记标题" value={title} onChange={(event) => { const nextTitle = event.target.value; setTitle(nextTitle); dirty(nextTitle, body); }} className="min-w-0 flex-1 bg-transparent text-base font-semibold tracking-[-0.01em] outline-none placeholder:text-[var(--text-tertiary)] sm:text-lg" placeholder="无标题笔记" /><span aria-live="polite" className={`hidden shrink-0 text-xs sm:inline ${state === "保存失败" || state === "版本冲突" ? "text-[var(--danger)]" : "text-[var(--text-tertiary)]"}`}>{state === "正在保存" ? "正在保存…" : state}</span><Button variant="ghost" size="sm" onClick={noteAiPanel.toggle} aria-label="打开笔记 AI" aria-pressed={noteAiPanel.isOpen}><Sparkles aria-hidden="true" /><span className="hidden sm:inline">AI</span></Button><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label="笔记更多操作"><MoreHorizontal aria-hidden="true" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => void save()}>立即保存 <span className="ml-auto text-xs text-[var(--text-tertiary)]">⌘S</span></DropdownMenuItem><DropdownMenuItem onSelect={() => void toggleFullscreen()}>{isFullscreen ? <Minimize2 aria-hidden="true" /> : <Maximize2 aria-hidden="true" />}{isFullscreen ? "退出全屏" : "全屏编辑"}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem disabled={isExporting} onSelect={() => { setPdfError(""); setPdfSnapshot({ title: title || "无标题笔记", body }); }}><Download aria-hidden="true" />{isExporting ? "正在生成 PDF…" : "导出 PDF"}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>{pdfError ? <p role="alert" className="px-4 py-2 text-sm text-[var(--danger)] sm:px-6">{pdfError}</p> : null}{imageUploadMessage ? <p role="status" aria-live="polite" className={`px-4 py-2 text-sm sm:px-6 ${imageUploadMessage.includes("失败") || imageUploadMessage.includes("不支持") ? "text-[var(--danger)]" : "text-[var(--success)]"}`}>{imageUploadMessage}</p> : null}<div className="min-h-0 flex-1 overflow-hidden bg-[var(--surface-canvas)]"><VisualMarkdownEditor markdown={body} noteId={note.id} onImageUploadStatus={setImageUploadMessage} onOpenAi={noteAiPanel.open} onSelectionChange={setSelection} onChange={handleBodyChange} /></div></div><NoteAiAssistant open={noteAiPanel.isOpen} onOpen={noteAiPanel.open} onClose={noteAiPanel.close} noteId={note.id} title={title} bodyMarkdown={body} defaultModel={noteAiDefaultModel} selection={selection} onReplaceNote={(suggestion) => { setBody(suggestion); dirty(title, suggestion); }} onInsertNote={(suggestion) => { const nextBody = `${body}${body.trim() ? "\n\n" : ""}${suggestion}`; setBody(nextBody); dirty(title, nextBody); }} />{pdfSnapshot ? <article id="note-pdf-preview" ref={pdfPreviewRef} className="fixed -left-[10000px] top-0 w-[794px] bg-white p-12 text-[15px]"><h1 className="mb-7 text-3xl font-semibold text-zinc-900">{pdfSnapshot.title}</h1><MarkdownDocument body={pdfSnapshot.body} /></article> : null}</section>;
}
