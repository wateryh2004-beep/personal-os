...
import { FilePlus2, FolderPlus, MoreHorizontal, Pin, Search, Star, Trash2 } from "lucide-react";
...
export function NotesWorkspace({ notes, folders, timezone, state, selectedFolder, initialView, dailyError }: {
  ...
  const title = selectedFolder?.name ?? (initialView === "favorites" ? "收藏" : "全部笔记");
  ...
  return (
    <section className="grid min-h-[calc(100dvh-9rem)] gap-8 lg:grid-cols-[252px_minmax(0,1fr)]">
      <aside className="self-start lg:sticky lg:top-6">
        ...
        <label className="relative mb-4 block">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索笔记" className="h-8 w-full rounded-md border bg-white pl-8 pr-2 text-sm placeholder:text-zinc-400 focus:border-[#365F78]" />
        </label>
      ...
      <main className="min-w-0 max-w-[1000px]">
