...
export function NotesWorkspace({ notes, folders, timezone, state, selectedFolder, initialView, dailyError, initialHasMore }: { notes: NoteListItem[]; folders: Folder[]; timezone: string; state: WorkspaceState; selectedFolder: Folder | null; initialView: "all" | "favorites" | "recent"; dailyError: boolean; initialHasMore: boolean }) {
  ...
  return <main ref={listScrollRef} className="workspace-scroll h-full overflow-y-auto bg-[var(--surface-canvas)] px-4 pt-14 pb-5 md:pt-5 sm:px-7 lg:px-10">
    <div className="mx-auto max-w-5xl">
      ...
      <header className="flex flex-wrap items-center gap-2 border-b pb-4"><h1 className="mr-auto text-xl font-semibold tracking-tight text-[var(--text-primary)]">{title}<span className="ml-2 text-sm font-normal text-[var(--text-tertiary)]">· {visible.length}</span></h1><AskNotesButton onClick={() => router.push("/notes/ask")} /></header>
