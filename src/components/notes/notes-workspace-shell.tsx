...
export function NotesWorkspaceShell({ folders, notes, children }: { folders: NotesNavigatorFolder[]; notes: NotesNavigatorNote[]; children: React.ReactNode }) {
  ...
  return <section className="flex h-[calc(var(--app-viewport-height)-var(--toolbar-height))] min-h-0 overflow-hidden bg-[var(--surface-canvas)]">
    ...
    <div className="relative min-w-0 flex-1"><button type="button" onClick={() => setMobileOpen(true)} className="absolute left-3 top-3 z-10 flex size-8 items-center justify-center rounded bg-[var(--surface-canvas)] text-[var(--text-secondary)] shadow-sm md:hidden" aria-label="打开笔记文件"><Menu className="size-4" /></button>{children}</div>
  </section>;
}
