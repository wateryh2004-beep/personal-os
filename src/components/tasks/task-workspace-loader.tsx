...
export function TaskWorkspaceLoader({ initialCreateOpen = false, initialTaskId }: { initialCreateOpen?: boolean; initialTaskId?: string }) {
  ...
  useEffect(() => {
    perfMark("workspace-visible", { workspace: "tasks", cached: Boolean(snapshot.data) });
    void tasksWorkspaceResource.revalidate().then(() => perfMeasure("workspace-data-ready", "navigation-click", { workspace: "tasks" })).catch(() => {});
  // The resource owns deduplication; this intentionally runs once on mount.
