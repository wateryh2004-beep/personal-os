...
export function StopGeocode({ tripId, stopId, placeName, onResolved }: { tripId: string; stopId: string; placeName: string; onResolved?: (item: Result) => void }) {
  const [query, setQuery] = useState(placeName); const [results, setResults] = useState<Result[]>([]); const [error, setError] = useState(""); const [pending, start] = useTransition();
  ...
  const choose = (item: Result) => { setError(""); start(async () => { try { await resolveTripStop(tripId, stopId, item); onResolved?.(item); setResults([]); } catch { setError("地点保存失败，请重试。"); } }); };
