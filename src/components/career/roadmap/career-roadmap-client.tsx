...
import { addDays, dateDiffDays, dateToX, getDurationLabelPosition, getTimelineDomain, getVisibleItemGeometry, isDuration, monthCount, timelineMonths, xToDate } from "@/features/career/roadmap-utils";
...
export function CareerRoadmapClient({ tracks, milestones, directions }: { tracks: RoadmapTrack[]; milestones: RoadmapMilestone[]; directions: Direction[] }) {
  ...
  }, [monthWidth, todayX, updateTimelineViewport]); // current month is always the initial viewport anchor
...
function TimelineLane({ track, items, domain, monthWidth, timelineWidth, timelineViewport, todayX, horizonX, drag, onDrag, onOpenMilestone, onOpenTrack, onCreate, onTrackDragStart, onTrackDrop }: { track: RoadmapTrack; items: RoadmapMilestone[]; domain: ReturnType<typeof getTimelineDomain>; monthWidth: number; timelineWidth: number; timelineViewport: TimelineViewport; todayX: number; horizonX: number; drag: Drag | null; onDrag: (value: Drag) => void; onOpenMilestone: (item: RoadmapMilestone) => void; onOpenTrack: (item: RoadmapTrack) => void; onCreate: (date: string) => void; onTrackDragStart: () => void; onTrackDrop: () => void }) {
...
function TimelineItem({ item, track, row, domain, monthWidth, timelineViewport, drag, onDrag, onOpen }: { item: RoadmapMilestone; track: RoadmapTrack; row: number; domain: ReturnType<typeof getTimelineDomain>; monthWidth: number; timelineViewport: TimelineViewport; drag: Drag | null; onDrag: (value: Drag) => void; onOpen: () => void }) {
  ...
  const label = getDurationLabelPosition({ barLeft: left, barRight: left + width, viewportLeft: timelineViewport.left, viewportRight: timelineViewport.right, padding: visible.clippedLeft ? 17 : 7 });
  const continuation = `${visible.clippedLeft ? "\n← 从当前视图之前开始" : ""}${visible.clippedRight ? "\n→ 延续到当前视图之后" : ""}`;
  return <div onPointerDown={(event) => begin(event, "move")} onClick={onOpen} title={`${item.title}\n${startsOn} — ${targetDate}\n${item.status === "completed" ? "已完成" : "进行中"}${continuation}${item.description ? `\n${item.description}` : ""}`} className={`absolute z-10 h-8 cursor-grab rounded-md border text-xs font-medium active:cursor-grabbing ${completed ? "opacity-50" : ""}`} style={{ left, top: row * 48 + 12, width, borderColor: accent[track.color], backgroundColor: `${accent[track.color]}12`, color: accent[track.color] }}><span onPointerDown={(event) => begin(event, "start")} className="absolute inset-y-0 left-0 z-20 w-1.5 cursor-ew-resize" />{visible.clippedLeft ? <span aria-hidden className="pointer-events-none absolute top-0 z-10 leading-8" style={{ left: Math.max(2, label.left - 11) }}>←</span> : null}{label.intersectsViewport && label.maxWidth > 18 ? <span className="pointer-events-none absolute top-0 z-10 block truncate leading-8 text-zinc-700" style={{ left: label.left, maxWidth: Math.max(0, label.maxWidth - (visible.clippedRight ? 12 : 0)) }}>{item.title}</span> : null}{visible.clippedRight ? <span aria-hidden className="pointer-events-none absolute top-0 z-10 leading-8" style={{ left: Math.max(0, Math.min(width - 12, timelineViewport.right - left - 10)) }}>→</span> : null}<span onPointerDown={(event) => begin(event, "end")} className="absolute inset-y-0 right-0 z-20 w-1.5 cursor-ew-resize" /></div>;
}
