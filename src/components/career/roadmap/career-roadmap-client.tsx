...
function TimelineMilestone({ item, rowItems, track, row, domain, monthWidth, timelineWidth, timelineViewport, todayDate, drag, onDrag, onOpen }: { item: RoadmapMilestone; rowItems: RoadmapMilestone[]; track: RoadmapTrack; row: number; domain: ReturnType<typeof getTimelineDomain>; monthWidth: number; timelineWidth: number; timelineViewport: TimelineViewport; todayDate: string; drag: Drag | null; onDrag: (value: Drag) => void; onOpen: () => void }) {
  ...
  if (!duration) {
    const desiredWidth = Math.min(192, Math.max(76, item.title.length * 13 + 24));
    const obstacles = rowItems
      .filter((candidate) => candidate.id !== item.id && isDuration(candidate))
      .map((candidate) => ({ left: dateToX(candidate.starts_on!, domain, monthWidth), right: dateToX(candidate.target_date, domain, monthWidth) + 5 }));
    const placement = getPointLabelPlacement({ pointX: left, desiredWidth, obstacles, timelineWidth });
    const labelVisible = placement.width >= 56;
    const pointStyle = { left: placement.side === "left" ? left - placement.width : left, top: row * 54 + 28, width: placement.width, color: accent[track.color] };
    const label = <span className="min-w-0"><span className={`flex items-center gap-1 font-medium text-zinc-700 ${item.status === "skipped" ? "line-through" : ""}`}><span className="truncate">{item.title}</span>{completed ? <Check className="size-3 shrink-0" /> : null}</span><span className={`mt-0.5 block font-mono text-[9px] ${temporalState === "past_unresolved" ? "text-zinc-500" : "text-zinc-400"}`}>{meta}</span></span>;
    const marker = <span className={`mt-1 size-2 shrink-0 rotate-45 border ${temporalState === "today" ? "ring-2 ring-[#365F78]/20" : ""}`} style={{ borderColor: accent[track.color], backgroundColor: completed ? accent[track.color] : "white" }} />;
    return <button onPointerDown={(event) => begin(event, "move")} onClick={onOpen} title={`${item.title} · ${targetDate}\n${temporalState === "past_unresolved" ? "历史计划日期，状态待确认" : item.status}${item.description ? `\n${item.description}` : ""}`} className={`absolute z-10 grid cursor-grab items-start gap-x-1.5 text-left text-xs active:cursor-grabbing ${placement.side === "left" ? "grid-cols-[minmax(0,1fr)_10px]" : "grid-cols-[10px_minmax(0,1fr)]"} ${resolved ? "opacity-50" : ""}`} style={pointStyle}>{placement.side === "left" ? <>{labelVisible ? label : null}{marker}</> : <>{marker}{labelVisible ? label : null}</>}</button>;
  }
