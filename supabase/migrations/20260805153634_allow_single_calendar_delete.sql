-- A delete request must always target one explicit provider event. It enters
-- the same audited, user-confirmed transition path as a create request.
...
begin
  if tg_op = 'INSERT' then
    if new.status = 'pending_confirmation' and new.operation_type in ('create', 'delete') then return new; end if;
