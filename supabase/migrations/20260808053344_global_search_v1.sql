...
create or replace function public.search_personal_os(p_query text,p_limit integer default 30,p_domains text[] default null) returns table(domain text,entity_type text,entity_id uuid,title text,subtitle text,snippet text,metadata jsonb,source_updated_at timestamptz,score real) language sql security invoker set search_path=public as $$ select s.domain,s.entity_type,s.entity_id,s.title,s.subtitle,left(regexp_replace(s.content_text,'[#*_\[\]]','','g'),240),s.metadata,s.source_updated_at,((case when lower(s.title)=lower(trim(p_query)) then 100 when s.title ilike trim(p_query)||'%' then 40 when s.title ilike '%'||replace(replace(replace(trim(p_query),'\\','\\\\'),'%','\\%'),'_','\\_')||'%' escape '\\' then 20 else 0 end)+ts_rank_cd(s.search_vector,plainto_tsquery('simple',trim(p_query))) * 20+(case when s.content_text ilike '%'||replace(replace(replace(trim(p_query),'\\','\\\\'),'%','\\%'),'_','\\_')||'%' escape '\\' then 5 else 0 end))::real from public.search_documents s where length(trim(p_query))>0 and (p_domains is null or s.domain=any(p_domains)) and (s.search_vector @@ plainto_tsquery('simple',trim(p_query)) or s.title ilike '%'||replace(replace(replace(trim(p_query),'\\','\\\\'),'%','\\%'),'_','\\_')||'%' escape '\\' or s.content_text ilike '%'||replace(replace(replace(trim(p_query),'\\','\\\\'),'%','\\%'),'_','\\_')||'%' escape '\\') order by score desc,s.source_updated_at desc nulls last limit greatest(1,least(p_limit,50)); $$;
